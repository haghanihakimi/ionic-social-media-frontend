...
const GuestProfileImagePosts: React.FC = () => {
  ...
  const samplePosts = [
    ...
    },
    {
      ...
      description: 'Post 41 description',
      created_at: '2024-01-28 21:53:34',
    },
  ];
  ...
  useEffect(() => {
    const updateParentWidth = () => {
      const width = parseInt((((window.innerWidth) / 4) - 2).toFixed(0), 10);
  ...
  return (
    <div id='guest-posts' className='w-full'>
      <div className='w-full flex gap-0 flex-row flex-wrap overflow-auto overflow-x-hidden'>
        <div className="w-full flex flex-row items-center justify-start gap-[2px] flex-wrap">
          {
            samplePosts && samplePosts.length > 0 ?
              samplePosts.map((post: any, i: number) => {
                const isLastTileInRow = (i + 1) % 4 === 0;
                return <div className={`block bg-gray-300 relative ${isLastTileInRow ? 'grow' : 'grow-0'}`} style={{ height: parentWidth, width: parentWidth }}
                  key={i}>
